...
def make_positions(tensor):
  mask = tf.cast(tf.math.not_equal(tensor, 0), tf.int32)  #Mask for removing pad elements                                                                              
  return tf.math.cumsum(mask, axis=1) * mask                                                                     
...
class LearnedPositionalEmbedding(tf.keras.layers.Embedding):
  ...
  def __init__(self,
    ...
    super().__init__(num_embeddings, 
                     embedding_dim, 
                     embeddings_initializer = tf.compat.v1.random_normal_initializer(
                                                                          mean=0.0, stddev=embedding_dim ** -0.5),
                     mask_zero = True)

  def call(self, input, positions=None):
    if positions is None:
        positions = make_positions(input)
    return super().call(positions)

def sin_positional_encoding(length,
                          hidden_size,
                          min_timescale=1.0,
                          max_timescale=1.0e4):
...
def positional_embedding(num_embeddings: int,
                        embedding_dim: int,
                        learned: bool = False):
  if learned:
  ...
  #Sinusoidal positional encoding
    m = sin_positional_encoding(num_embeddings, embedding_dim)
  return m
